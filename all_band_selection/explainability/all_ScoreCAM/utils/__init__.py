...
# funzione per il mio caso specifico
def find_cnn1d_layer(arch, target_layer_name):
    """
    Cerca un layer in una CNN 1D personalizzata supportando
    nomi e indici numerici (es. 'features_conv.4').
    """
    # Divide la stringa 'features_conv.4' in ['features_conv', '4']
    hierarchy = target_layer_name.split('.')
    target_layer = arch

    for path in hierarchy:
        if path.isdigit():
            # Se è un numero (es. 4), accediamo per indice (per nn.Sequential)
            target_layer = target_layer[int(path)]
        else:
            # Se è un nome (es. features_conv), accediamo per attributo
            if hasattr(target_layer, path):
                target_layer = getattr(target_layer, path)
            elif path in target_layer._modules:
                target_layer = target_layer._modules[path]
            else:
                raise ValueError(f"Impossibile trovare il layer '{path}' in {target_layer}")
        print("target_layer")
        print(target_layer)
        print("************************")
    return target_layer
